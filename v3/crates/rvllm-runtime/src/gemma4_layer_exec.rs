    ...
    match phase {
        ...
        Gemma4Phase::Prefill { cu_seqlens_q, max_seqlen_q, num_seqs: _ } => {
            if dims.kv_dtype == KvDtype::Nvfp4 {
                // NVFP4 prefill: single-kernel path (no unified / no
                // decode-per-qi fallback) — the NVFP4 launcher handles
                // a full batch in one call.
                rope_nvfp4kv(dims, kernels, scratch, meta, stream)?;
            ...
            // FP8 / F16 prefill: shares the FP8 write path (there is no
            // F16 prefill kernel). Rest of this arm is the HEAD (F-series)
            // unified-or-fallback structure.
            rope_fp8kv(dims, kernels, scratch, meta, stream)?;
