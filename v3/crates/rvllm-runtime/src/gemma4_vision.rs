...
pub struct Gemma4VisionRuntime<'a> {
    ...
    /// Optional bf16 weight cache for the bf16 forward path.
    /// Populated by the per-bringup `ensure_vit_bf16_weights` lazy
    /// initializer before `forward_bf16` is called. `None` ⇒
    /// `forward_bf16` errors out with a clear "weights not populated"
    /// message. Production `forward` (f16) never touches this field.
    pub bf16_weights: Option<&'a Gemma4VisionBf16>,
}
...
impl<'a> Gemma4VisionRuntime<'a> {
    ...
    /// Phase-3 bf16 vision forward — body landed 2026-05-22 as a
    /// mechanical kernel-by-kernel substitution of `forward`
    /// (gemma4_vision.rs:572) per the GEMMA_VISION_AUDIT.md
    /// "Replay recipe" plan. Every f16 kernel function pointer is
    /// swapped for its bf16 sibling on `self.fused_bf16`; every
    /// cuBLASLt `f16_gemm_f32{,_batched_strided}` call is swapped
    /// for `bf16_gemm_f32{,_batched_strided}`; every weight pointer
    /// reads from `self.bf16_weights` (lazy-populated at first
    /// call by `ensure_vit_bf16_weights` on each bringup); host-
    /// side `half::f16::from_f32` narrows become
    /// `half::bf16::from_f32`.
    ///
    /// Quality status (2026-05-22 landing): UNVALIDATED on
    /// hardware. The prior Phase-3 attempt failed at blk0_out
    /// cos=0.76 without localization; the per-substep dump
    /// harness now exists (`RVLLM_GEMMA4_VIT_SUBSTEP_BLK` +
    /// `cmp_g4v_substep.py`) to localize divergence within block 0
    /// — that's the next debug session's first action when the
    /// operator flips `RVLLM_GEMMA4_VIT_USE_BF16=1`.
    ///
    /// Production f16 path (`forward` above) remains the
    /// unconditional default; this body is reachable only via the
    /// explicit env gate.
    #[cfg(feature = "cuda")]
    pub fn forward_bf16(
        &self,
        ...
        let bf16k = self.fused_bf16.ok_or_else(|| {
            rvllm_core::RvllmError::cuda(
                "vision: bf16 kernel set not loaded (forward_bf16 \
                 reached without ensure_vit_bf16_weights succeeding)",
                rvllm_core::CudaErrorKind::Other,
                rvllm_core::CudaCtx::setup(),
            )
        })?;
        let bf16w = self.bf16_weights.ok_or_else(|| {
            rvllm_core::RvllmError::cuda(
                "vision: bf16 weight cache not populated (call \
                 ensure_vit_bf16_weights on the bringup first)",
                rvllm_core::CudaErrorKind::Other,
                rvllm_core::CudaCtx::setup(),
            )
        })?;
        ...
        // information was being attenuated which matches the observed
        // 'Layout erkannt, Buchstaben nicht gelesen' behaviour.
        let mut patches_bf16 = Vec::with_capacity(pixel_active.len() * 2);
        for &x in &pixel_active {
            let y = 2.0 * x - 1.0;
            patches_bf16.extend_from_slice(&half::bf16::from_f32(y).to_le_bytes());
        }
        let patches_region = self.arena.region("g4vbf16_patches", patches_bf16.len(), 16)?;
        unsafe { patches_region.copy_from_host(&patches_bf16)? };
        ...
        let row_pos_region = self.arena.region("g4vbf16_rowpos", n_tokens * 4, 16)?;
        let col_pos_region = self.arena.region("g4vbf16_colpos", n_tokens * 4, 16)?;
        unsafe { row_pos_region.copy_from_host(row_pos_bytes)? };
        unsafe { col_pos_region.copy_from_host(col_pos_bytes)? };
        ...
        let linear_no_bias = |in_dev: u64,
         ...
         -> Result<()> {
            ...
            unsafe {
                ...
                let grid = ((n_elem as u32 + block - 1) / block, 1u32, 1u32);
                let rc = cuLaunchKernel(
                    bf16k.fn_cast_f32_to_bf16.raw() as CUfunction,
                    grid.0, grid.1, grid.2,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        let rmsnorm = |x: u64, gamma: u64, rows: usize, dim: usize| -> Result<()> {
            ...
            unsafe {
                ...
                let block: u32 = (dim as u32).min(1024);
                let rc = cuLaunchKernel(
                    bf16k.fn_rmsnorm_bf16_gbf16.raw() as CUfunction,
                    rows as u32, 1, 1,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        let vnorm = |x: u64, rows: usize, dim: usize| -> Result<()> {
            ...
            unsafe {
                ...
                let block: u32 = (dim as u32).min(1024);
                let rc = cuLaunchKernel(
                    bf16k.fn_vnorm_bf16.raw() as CUfunction,
                    rows as u32, 1, 1,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        let device_residual_add = |dst: u64, src: u64| -> Result<()> {
            ...
            unsafe {
                ...
                let grid = ((n_elem as u32 + block - 1) / block, 1u32, 1u32);
                let rc = cuLaunchKernel(
                    bf16k.fn_vector_add_bf16.raw() as CUfunction,
                    grid.0, grid.1, grid.2,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        let dump_stage =
            |name: &str, dev_ptr: u64, n_rows: usize, ncols: usize| -> Result<()> {
                if let Some(dir) = dump_dir.as_ref() {
                    ...
                    let path = dir.join(format!("g4vbf16_{name}.bin"));
                    if let Err(e) = std::fs::write(&path, &host) {
                        eprintln!("[g4v-audit] write {path:?}: {e}");
                    } else {
                        eprintln!(
        ...
        let dump_substep =
            |blk_idx: usize, name: &str, dev_ptr: u64, n_rows: usize, ncols: usize| -> Result<()> {
                ...
                if let Some(dir) = dump_dir.as_ref() {
                    ...
                    let path = dir.join(format!("g4vbf16_blk{blk_idx}_{name}.bin"));
                    if let Err(e) = std::fs::write(&path, &host) {
                        eprintln!("[g4v-substep] write {path:?}: {e}");
                    } else {
                        eprintln!("[g4v-substep] blk{blk_idx}/{name} → {path:?} ({n_rows}, {ncols}) f16");
        ...
        // ── Step 1: patch_embed (linear, no bias) [N, 768] → [N, 1152] ─
        let hidden_bytes = n_tokens * HIDDEN * 2;
        let hidden_region = self.arena.region("g4vbf16_hidden", hidden_bytes, 16)?;
        linear_no_bias(
            patches_region.device_ptr(),
            bf16w.patch_embedder_input_proj_ptr,
            hidden_region.device_ptr(),
            n_tokens, HIDDEN, 768,
        )?;
        dump_stage("patch_embed_linear", hidden_region.device_ptr(), n_tokens, HIDDEN)?;
        ...
        unsafe {
            ...
            // and row_pos as axis_1. Codex review #2 caught the swap.
            let mut h = hidden_region.device_ptr();
            let mut tab = bf16w.patch_embedder_pos_table_ptr;
            let mut a0 = col_pos_region.device_ptr();
            let mut a1 = row_pos_region.device_ptr();
            let mut np = NUM_POS as i32;
            let mut hd = HIDDEN as i32;
            ...
            let rc = cuLaunchKernel(
                bf16k.fn_vit_pos_emb_lookup_2d_bf16.raw() as CUfunction,
                n_tokens as u32, 1, 1,
                256, 1, 1,
                0, self.stream.raw() as CUstream,
                args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        for t in 0..n_tokens {
            ...
            for k in 0..inv_freq_dim {
                let a_col = col * inv_theta[k];
                let a_row = row * inv_theta[k];
                let cos_col = half::bf16::from_f32(a_col.cos()).to_le_bytes();
                let sin_col = half::bf16::from_f32(a_col.sin()).to_le_bytes();
                let cos_row = half::bf16::from_f32(a_row.cos()).to_le_bytes();
                let sin_row = half::bf16::from_f32(a_row.sin()).to_le_bytes();
                let rb = t * HEAD_DIM;
                // Chunk 0 (channels [0, 36)): col-axis, mirrored at offset 18.
                for &mirror in &[0usize, inv_freq_dim] {
                    let off = (rb + mirror + k) * 2;
        ...
        let cos_region = self.arena.region("g4vbf16_cos", cos_host.len(), 16)?;
        let sin_region = self.arena.region("g4vbf16_sin", sin_host.len(), 16)?;
        unsafe {
            cos_region.copy_from_host(&cos_host)?;
            sin_region.copy_from_host(&sin_host)?;
        }
        ...
        // ── Step 4: 27-block sandwich-norm encoder loop. ─────────────
        let normed = self.arena.region("g4vbf16_normed", n_tokens * HIDDEN * 2, 16)?;
        let q_buf = self.arena.region("g4vbf16_q", n_tokens * HIDDEN * 2, 16)?;
        let k_buf = self.arena.region("g4vbf16_k", n_tokens * HIDDEN * 2, 16)?;
        let v_buf = self.arena.region("g4vbf16_v", n_tokens * HIDDEN * 2, 16)?;
        let attn_out = self.arena.region("g4vbf16_attn_out", n_tokens * HIDDEN * 2, 16)?;
        let proj_out = self.arena.region("g4vbf16_proj_out", n_tokens * HIDDEN * 2, 16)?;
        // Batched-strided attention scratch (Codex review #B round 3 /
        // #4 round 4 follow-up). One [H, N, N] / [H, N, D] / [H, D, N]
        // slab each — heads packed contiguously so cuBLASLt can run the
        // 16-head QK^T and (scores @ V) GEMMs as a single strided-batch
        ...
        let scores_buf_all = self
            .arena
            .region("g4vbf16_scores_all", NUM_HEADS * n_tokens * n_tokens * 2, 16)?;
        let scores_f32_all = self
            .arena
            .region("g4vbf16_scores_f32_all", NUM_HEADS * n_tokens * n_tokens * 4, 16)?;
        let v_t_all = self
            .arena
            .region("g4vbf16_vt_all", NUM_HEADS * HEAD_DIM * n_tokens * 2, 16)?;
        let out_f32_all = self
            .arena
            .region("g4vbf16_out_f32_all", NUM_HEADS * n_tokens * HEAD_DIM * 4, 16)?;
        let out_hmajor = self
            .arena
            .region("g4vbf16_out_hmajor", NUM_HEADS * n_tokens * HEAD_DIM * 2, 16)?;
        let mlp_g = self.arena.region("g4vbf16_mlp_g", n_tokens * INTERMEDIATE * 2, 16)?;
        let mlp_u = self.arena.region("g4vbf16_mlp_u", n_tokens * INTERMEDIATE * 2, 16)?;
        let mlp_d = self.arena.region("g4vbf16_mlp_d", n_tokens * HIDDEN * 2, 16)?;

        for (blk_idx, (blk, bf16_blk)) in vision.blocks.iter()
            .zip(bf16w.blocks.iter()).enumerate() {
            // === ATTENTION sub-block (sandwich norm) =====================
            // residual_attn = hidden;
            // x = input_layernorm(hidden)
            // x = self_attn(x)
            ...
            self.stream.fence()?;
            rmsnorm(normed.device_ptr(), bf16_blk.input_layernorm_w_ptr, n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "input_ln", normed.device_ptr(), n_tokens, HIDDEN)?;

            // q/k/v projections (no bias).
            linear_no_bias(normed.device_ptr(), bf16_blk.q_proj_w_ptr,
                q_buf.device_ptr(), n_tokens, HIDDEN, HIDDEN)?;
            linear_no_bias(normed.device_ptr(), bf16_blk.k_proj_w_ptr,
                k_buf.device_ptr(), n_tokens, HIDDEN, HIDDEN)?;
            linear_no_bias(normed.device_ptr(), bf16_blk.v_proj_w_ptr,
                v_buf.device_ptr(), n_tokens, HIDDEN, HIDDEN)?;
            dump_substep(blk_idx, "q_proj", q_buf.device_ptr(), n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "k_proj", k_buf.device_ptr(), n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "v_proj", v_buf.device_ptr(), n_tokens, HIDDEN)?;
            ...
            // q_norm / k_norm: per-head RMSNorm head_dim=72 (gammas
            // pre-shifted in checkpoint). View Q/K as [N*num_heads, 72].
            rmsnorm(q_buf.device_ptr(), bf16_blk.q_norm_w_ptr,
                n_tokens * NUM_HEADS, HEAD_DIM)?;
            rmsnorm(k_buf.device_ptr(), bf16_blk.k_norm_w_ptr,
                n_tokens * NUM_HEADS, HEAD_DIM)?;
            ...
            for &qk_ptr in &[q_buf.device_ptr(), k_buf.device_ptr()] {
                ...
                unsafe {
                    ...
                    let rc = cuLaunchKernel(
                        bf16k.fn_vit_rotary_gemma4_2d_bf16.raw() as CUfunction,
                        n_tokens as u32, NUM_HEADS as u32, 1,
                        (HEAD_DIM / 4) as u32, 1, 1,   // 18 threads = chunk_size/2
                        0, self.stream.raw() as CUstream,
                        args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            let _extract_head = |dst: u64, src: u64, head_idx: usize| -> Result<()> {
                ...
                unsafe {
                    ...
                    let rc = cuLaunchKernel(
                        bf16k.fn_extract_head_bf16.raw() as CUfunction,
                        n_tokens as u32, 1, 1,
                        HEAD_DIM as u32, 1, 1,
                        0, self.stream.raw() as CUstream,
                        args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            let _scatter_head = |dst: u64, src: u64, head_idx: usize| -> Result<()> {
                ...
                unsafe {
                    ...
                    let rc = cuLaunchKernel(
                        bf16k.fn_scatter_head_bf16.raw() as CUfunction,
                        n_tokens as u32, 1, 1,
                        HEAD_DIM as u32, 1, 1,
                        0, self.stream.raw() as CUstream,
                        args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            #[cfg(feature = "cuda")]
            unsafe {
                self.cublaslt.bf16_gemm_f32_batched_strided(
                    q_buf.device_ptr(),
                    k_buf.device_ptr(),
                    scores_f32_all.device_ptr(),
                    n_tokens as i32,                // m = N (Q rows)
            ...
            unsafe {
                ...
                let block: u32 = (n_tokens as u32).min(1024);
                let rc = cuLaunchKernel(
                    bf16k.fn_softmax_row_f32_to_bf16.raw() as CUfunction,
                    (n_tokens * NUM_HEADS) as u32, 1, 1,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            unsafe {
                ...
                let rc = cuLaunchKernel(
                    bf16k.fn_transpose_heads_v_bf16.raw() as CUfunction,
                    n_tokens as u32, NUM_HEADS as u32, 1,
                    HEAD_DIM as u32, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            unsafe {
                ...
                let rc = cuLaunchKernel(
                    bf16k.fn_scatter_heads_bf16.raw() as CUfunction,
                    n_tokens as u32, NUM_HEADS as u32, 1,
                    HEAD_DIM as u32, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            // o_proj (no bias).
            linear_no_bias(attn_out.device_ptr(), bf16_blk.o_proj_w_ptr,
                proj_out.device_ptr(), n_tokens, HIDDEN, HIDDEN)?;
            dump_substep(blk_idx, "o_proj", proj_out.device_ptr(), n_tokens, HIDDEN)?;

            // post_attention_layernorm on proj_out.
            rmsnorm(proj_out.device_ptr(), bf16_blk.post_attention_layernorm_w_ptr,
                n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "post_attn_ln", proj_out.device_ptr(), n_tokens, HIDDEN)?;
            ...
            self.stream.fence()?;
            rmsnorm(normed.device_ptr(), bf16_blk.pre_feedforward_layernorm_w_ptr,
                n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "pre_ff_ln", normed.device_ptr(), n_tokens, HIDDEN)?;

            linear_no_bias(normed.device_ptr(), bf16_blk.gate_proj_w_ptr,
                mlp_g.device_ptr(), n_tokens, INTERMEDIATE, HIDDEN)?;
            linear_no_bias(normed.device_ptr(), bf16_blk.up_proj_w_ptr,
                mlp_u.device_ptr(), n_tokens, INTERMEDIATE, HIDDEN)?;
            dump_substep(blk_idx, "gate_proj", mlp_g.device_ptr(), n_tokens, INTERMEDIATE)?;
            dump_substep(blk_idx, "up_proj", mlp_u.device_ptr(), n_tokens, INTERMEDIATE)?;
            ...
            unsafe {
                ...
                let grid = ((n_elem as u32 + block - 1) / block, 1u32, 1u32);
                let rc = cuLaunchKernel(
                    bf16k.fn_gelu_tanh_mul_bf16.raw() as CUfunction,
                    grid.0, grid.1, grid.2,
                    block, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
            ...
            linear_no_bias(mlp_g.device_ptr(), bf16_blk.down_proj_w_ptr,
                mlp_d.device_ptr(), n_tokens, HIDDEN, INTERMEDIATE)?;
            dump_substep(blk_idx, "down_proj", mlp_d.device_ptr(), n_tokens, HIDDEN)?;

            // post_feedforward_layernorm on mlp_d.
            rmsnorm(mlp_d.device_ptr(), bf16_blk.post_feedforward_layernorm_w_ptr,
                n_tokens, HIDDEN)?;
            dump_substep(blk_idx, "post_ff_ln", mlp_d.device_ptr(), n_tokens, HIDDEN)?;
        ...
        // (~2752) was multiplied by sqrt(1152) ≈ 33.94.
        let pooled_region_f32 =
            self.arena.region("g4vbf16_pooled_f32", n_pooled * HIDDEN * 4, 16)?;
        let pooled_region = self.arena.region("g4vbf16_pooled", n_pooled * HIDDEN * 2, 16)?;
        ...
        unsafe {
            ...
            let rc = cuLaunchKernel(
                bf16k.fn_vit_avgpool_bf16_to_f32.raw() as CUfunction,
                n_pooled as u32, 1, 1,
                256, 1, 1,
                0, self.stream.raw() as CUstream,
                args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        #[cfg(feature = "cuda")]
        if let (Some(std_bias), Some(std_scale)) =
            (bf16w.std_bias_ptr.as_ref(), bf16w.std_scale_ptr.as_ref())
        {
            unsafe {
                let mut out = pooled_region.device_ptr();
                let mut x = pooled_region_f32.device_ptr();
                let mut bias = *std_bias;
                let mut scale = *std_scale;
                let mut hd = HIDDEN as i32;
                let args = [
                    (&mut out) as *mut u64 as *mut core::ffi::c_void,
                    (&mut x) as *mut u64 as *mut core::ffi::c_void,
                ...
                let rc = cuLaunchKernel(
                    bf16k.fn_vit_standardize_f32_to_bf16.raw() as CUfunction,
                    n_pooled as u32, 1, 1,
                    256, 1, 1,
                    0, self.stream.raw() as CUstream,
                    args.as_ptr() as *mut *mut core::ffi::c_void,
        ...
        // Linear [HIDDEN → OUT_HIDDEN] no bias.
        let final_region = self.arena.region("g4vbf16_final", n_pooled * OUT_HIDDEN * 2, 16)?;
        linear_no_bias(
            pooled_region.device_ptr(),
            bf16w.embed_vision_projection_ptr,
            final_region.device_ptr(),
            n_pooled, OUT_HIDDEN, HIDDEN,
        )?;
        dump_stage("post_projection", final_region.device_ptr(), n_pooled, OUT_HIDDEN)?;
