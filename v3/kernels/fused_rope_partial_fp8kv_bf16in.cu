// Partial RoPE + FP8 paged-KV-cache write (Gemma 4) — BF16-input variant.
//
// Cycle 55 step 7 (Phase B): bf16-input sibling of
// fused_rope_partial_fp8kv_kernel. Identical math (per-token Q scale,
// per-slot-per-head K/V scales, partial RoPE on `rotary_dim` of
// `head_dim` lanes); only Q/K/V activation inputs flip
// __half → __nv_bfloat16 with __half2float → __bfloat162float at
// each load.
//
// cos_table + sin_table stay __half — they are read-only model
// constants the loader currently materializes as f16, and the
// conversion is a single PTX cvt instruction at the cache-resident
// fast path. Phase D (norm gammas + small constants → bf16 at load)
// will revisit those.
//
// Output dtypes are unchanged: q_fp8_out / key_cache / value_cache
// are FP8 E4M3 by design (the whole point of the fp8kv path); the
// activation-side dtype change does not propagate here.
//
...
// correctly doing. (When `q_scale_cache` is non-null, switches to
// per-(token,head) dynamic Q scaling.)
//
...
float block_max_abs_bf16in(float x, int half_head, int tid, float* warp_max) {
    #pragma unroll
...
extern "C"
__global__ void fused_rope_partial_fp8kv_bf16in_kernel(
    ...
    float* __restrict__ k_scale_cache,    // [num_slots * num_kv_heads] f32
    float* __restrict__ v_scale_cache,    // [num_slots * num_kv_heads] f32
    float* __restrict__ q_scale_cache,    // [num_tokens * num_heads] f32
    const __half* __restrict__ cos_table, // model constants stay f16
    ...
    if (head_idx < num_heads) {
        ...
        if (tid < half_rotary) {
            ...
            float q_lo = __bfloat162float(q_in[q_base + tid]);
            float q_hi = __bfloat162float(q_in[q_base + tid + half_head]);
            q_lo_val = q_lo * cos_val - q_hi * sin_val;
            q_hi_val = q_lo * sin_val + q_hi * cos_val;
        } else {
            q_lo_val = __bfloat162float(q_in[q_base + tid]);
            q_hi_val = __bfloat162float(q_in[q_base + tid + half_head]);
        }
        ...
        if (q_perblock) {
            ...
            float q_amax = block_max_abs_bf16in(
    ...
    if (head_idx < num_kv_heads) {
        ...
        if (tid < half_rotary) {
            float cos_val = __half2float(cos_table[pos * half_rotary + tid]);
            float sin_val = __half2float(sin_table[pos * half_rotary + tid]);
            float k_lo = __bfloat162float(k_in[k_base + tid]);
            float k_hi = __bfloat162float(k_in[k_base + tid + half_head]);
            k_lo_val = k_lo * cos_val - k_hi * sin_val;
            k_hi_val = k_lo * sin_val + k_hi * cos_val;
        } else {
            k_lo_val = __bfloat162float(k_in[k_base + tid]);
            k_hi_val = __bfloat162float(k_in[k_base + tid + half_head]);
        }
        float k_amax = block_max_abs_bf16in(
            fmaxf(fabsf(k_lo_val), fabsf(k_hi_val)),
            half_head, tid, warp_max);

        // --- Pass 2: compute V values, reduce to V amax. ---
        float v_lo_val = __bfloat162float(v_in[v_base + tid]);
        float v_hi_val = __bfloat162float(v_in[v_base + tid + half_head]);
        float v_amax = block_max_abs_bf16in(
