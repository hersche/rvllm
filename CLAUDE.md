...
| CUDA Graph capture validation (task #134) | **VALIDATED 2026-05-25** (`f1aea84`). Existing `forward_full_to_token_captured` infrastructure (from prior session) confirmed cross-request-safe via A/B/A2 test (coherent prompt-A → prompt-B → prompt-A again); capture log "[graph] captured 1685 nodes" confirms cuGraphLaunch path engages. New `clear_decode_capture` safety helper for future flip-on-the-fly support. Performance A/B on gemma4-nvfp4 dense decode (60-tok target, 100 max_new): GRAPH_OFF+INDIRECT_OFF=5.28 tok/s baseline, GRAPH_OFF+INDIRECT_ON=4.40 (-17%, INDIRECT alone hurts), GRAPH_ON+INDIRECT_ON=4.53 (wins 3% back but net loss). **Default stays OFF**: launch overhead is ~1.7% of step time on 31B dense — graph savings rounded off, INDIRECT prerequisite is net-negative. Same finding as qwen36 captured-graph ("+0.8% within noise"). Captured-graph is the right tool for small launch-bound models (qwen36 MoE) not 31B dense. Profile also bumps `RVLLM_REQUEST_TIMEOUT_SECS` 600→1500 to fix the cold-cache 599s timeout cascade on real zeroclaw turns. |
