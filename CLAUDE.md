...
| Spec-decode runaway → "Request timed out" (2026-05-29, **FIXED** `b4197f9`) | A turn with `max_tokens=8192` (zeroclaw default) emitted 4792 tokens (`bailout_tokens=4687`, `bailout_ms=1.38M` = 23 min) then SSE-timed-out. ROOT CAUSE: the spec-session **bailout decode-fill loops ignored `stop_token_ids`** — when spec bailed to plain `forward_full_to_token` decode (low accept), the loop ran to max_new even after the model emitted EOS. (The main spec loop checked stops; the two bailout subloops did not.) Secondary: no degenerate-loop guard, so a short repeating cycle at long context never self-terminated. Fix: both bailout subloops break on `stop_token_ids.contains(&next)`; added `emitted_tail_is_degenerate` (period-p≤8 cycle repeated ≥32×/p=1 or ≥12×/p≥2 — conservative, never trips on natural text incl. lists/code) wired into both spec loops + both bailout subloops (opt-out `G4N_SPEC_LOOP_GUARD=0`). Validated: 3 prompts at max_tokens=8192 all `finish=stop` at natural EOS (21/49/253 tok), no truncation, no runaway. NB: split-KV decode (below) was already working at the time (`bailout_ms`/tok = 295 ms = the fast split speed); the timeout was purely the 4792-token count, not per-token speed. |
| Long-context decode: split-KV for the single-token path (2026-05-28, **SHIPPED** `ebbde5b`, ~1.9×) | The Option B single-token decode (`forward_layer_attn_from_residual_dev` — spec bailout + remaining-1 + K=1 paths) used the single-CTA-per-head decode kernel (grid `(1, num_heads, 1)`, one CTA sweeps the full ctx serially → ~533 ms/token @ 17k). The fp8-block path long had split-KV decode (paged_attention_v2: partition KV across CTAs + reduce, +75%) but Option B never got it. Wired it in: `ensure_split_decode_workspace(max_pos)` (persistent partition scratch above forward_checkpoint, 8.4 MB @ 32k/part256) + a gated branch in the decode helper that calls the validated `PagedDecodeNvfp4Launcher::launch_split` (bf16-out, per-layer sliding/global Fa2 backend, partition_offset=0) when ctx > partition. Validated (forced bailout, ~17k): **md5 IDENTICAL** split OFF vs ON (byte-coherent greedy) + bailout decode 11725→6296 ms (533→286 ms/tok = **−46%**); short-context skips split (single-CTA, coherent). DEFAULT-ON (opt-out `RVLLM_GEMMA4_NVFP4_SPLIT_DECODE=0`), matching fp8-block. Directly cuts the warm-turn bailout cost (was ~40s of a ~47s warm turn). NOTE: the K=7 spec MAIN path (drafter + unified-prefill verify) is a separate kernel set — the drafter's global cross-attn (`launch_cross_attn_global`, single-CTA, K=7 steps/iter) is the next target if full-spec long-context decode needs more. |
| Long-context decode cost localized — single-CTA decode at 16k (2026-05-28, ROOT-CAUSE, superseded by the SHIPPED entry above) | The warm-turn wall (~47 s for 80 tokens at 16k context, prefill cached to 0.4 s) is decode-bound at ~1.5-2 tok/s. Breakdown of a warm turn (`g4n-spec-timing`): `bailout_ms=39769` of `total_ms=46836` — 68 of 80 tokens went through the low-accept bailout = plain `forward_full_to_token` per-token decode at ~585 ms/token. Full-spec turns are similar (~2.1 tok/s): `drafter_ms` dominates (e.g. 21130 ms / 31 iters = **680 ms/iter** for a 4-LAYER drafter — MORE than the 60-layer verify's ~430 ms/iter) + `verify_ms`. ROOT CAUSE: every per-token base decode (`forward_full_to_token`, used by the bailout + K=1 paths) and the drafter's global cross-attn (`launch_cross_attn_global`, gemma4_drafter.rs:1511, K=7 sequential steps/iter) use the **single-CTA-per-head** decode kernel (`flash_attention_2_decode_nvfp4kv_{gqa_}bf16out`, grid `(1, num_heads, 1)`) — one CTA sweeps the full 16k KV serially, no KV-parallelism. The split-KV decode kernels EXIST (`flash_attention_split_decode_nvfp4kv_bf16out.cu` + `paged_attention_reduce_bf16` reducer, paged_attention_v2 style: grid `(S, H, num_partitions)` + reduce) but are **NOT wired into the Option B (gemma4-nvfp4) bring-up** — it only loads the single-CTA `fn_attn_decode_{gqa_}bf16out`. FIX (bounded ~200-300 LOC, env-gated, the highest-leverage remaining lever — the bailout single-token decode is 85% of warm-turn cost): load split + reduce kernels into the bring-up, allocate per-partition scratch (tmp_out [H,P,D] f16 + max_logits/exp_sums [H,P] f32, P=ceil(ctx/partition_size)), and dispatch split+reduce instead of single-CTA in `forward_full_to_token`'s decode attention when `ctx_len > partition_size`. Must byte-validate split+reduce ≈ single-CTA + A/B per-token decode at 16k. NOT yet implemented — flagged as the next focused, carefully-validated piece (rushing risky GPU dispatch on the production model is what caused the ncu OOM-reboot earlier this session). The cheap wins (K-flip, cancellation) already cut the long-context pain meaningfully. |
| Spec-session cancellation + adaptive-K-off + precheck/device-accept findings (2026-05-28) | **SHIPPED** (cancellation `713a084`, K-flip `bf1c84d`). Investigation of slow/stalling zeroclaw turns (codex review cross-checked). FINDINGS: (1) **Cancellation** — the K>=2/K=1 spec sessions were uncancellable, so after a provider/client timeout the single-in-flight worker kept generating the whole reply and wedged the queue (the "no spec_timing for 1200s" stall). Fixed: `nvfp4_cancel: Mutex<Option<Arc<AtomicBool>>>` + per-iteration-boundary checks in both sessions + both bailout decode-fill subloops; worker installs `req.cancelled` around the call. Validated: a max_new=400 gen disconnected at 5s broke at emitted=33 and the worker freed instantly (probe 0s vs ~40s queue). (2) **Adaptive-K OFF** — post-MMA sweep shows fixed K7 (31.65 tok/s) > adaptive (29.77), per-prompt −34%; at 27-43% accept adaptive collapsed K→1 which is wrong now the verify pass is MMA-cheap. Profile `G4N_SPEC_ADAPTIVE_K=0`. (3) **Precheck is NOT a doubled prefill** (codex's premise disproven by measurement): the reply-intent precheck shares the persona prefix, so the reply rides the prefix cache — measured precheck cold-prefill 206243 ms (27k persona) then reply prefill **476 ms (cache HIT)**. The precheck's real cost is ~one cheap forward/turn; a global disable would risk group-channel spam (loses NO_REPLY filter), so NOT disabled. (4) **Device-side accept compare — SKIPPED**: per-iter cost is the ~307 ms 60-layer verify forward (measured `verify_ms/iters`); the fence+DtoH it would remove is <0.3%, not worth spec-correctness risk. The remaining latency floor is the unavoidable cold prefill (~91 s @16k / ~206 s @27k) — only the cross-request prefix cache mitigates it on repeat turns; warm turns already hit <30 s (3.6 s reply measured). |
| Mobile 16k spec reply OOM → empty reply → Telegram "3 dots then deletes" (2026-05-28) | **FIXED** (profile). On Telegram/Matrix (`stream_mode=partial` → draft placeholder) a reply showed the "3 dots" draft then instantly deleted; worked on legacy vLLM. Root cause (reproduced): the mobile profile pinned `G4N_KV_MAX_POS=131072` (128k) at `RVLLM_ARENA_GB=75`; the 128k KV pre-alloc starved the per-request spec-decode scratch, so a ~16k-persona spec reply (max_tokens>1) hit `gemma4-nvfp4 spec session K=7: Cuda AllocFailed, HbmArena::region` → HTTP 500 → ZeroClaw got an empty reply → `finalize_draft` edited the Telegram draft with empty text → 400 "message text is empty" → delete+send fallback → also empty → placeholder deleted, nothing replaces it. NOT a streaming bug (streaming emits content fine), NOT the cprefill kernels (byte-identical), NOT aux-KV (OOM persists with `AUX_KV=0`). Fix: dropped mobile `G4N_KV_MAX_POS`+`RVLLM_MAX_TOKENS_CAP` 131072→**32768** in `~/.rvllm/profiles/mobile-31b-nvfp4w-rvllm-spec.env`. Validated: 16k spec reply (stream + non-stream) → HTTP 200 coherent German. gb10 base stays 64k; only the mobile 128k override was the problem. (NB: the long multi-iteration zeroclaw turn that hits the 1200s `provider_timeout_secs` is a SEPARATE GPU-contention slowness, not this OOM.) |
| Prefix-cache clobber by short aux requests (2026-05-28) | **SHIPPED** (`aa8457f`). The #133 cache is a single slot tracking the last request's (prompt+emitted), and ALL requests share ONE KV buffer overwritten front-to-back. A short divergent request interleaved between two conversation turns (health-check `hi`, reply-intent precheck, slash command) both clobbers the cache slot AND physically corrupts the conversation's KV head → the next 14-16k turn cold-re-prefills (~70s wasted). Traced live: a 14438-tok reply looked up against a 19-tok `hi` cache (lcp=2). Reproduced via direct API: BIG(513)→`hi`(14)→BIG-again gave lcp=4 (miss). **Fix**: a 16k SCRATCH KV region (`allocate_aux_kv_state` — sanctioned 2nd-region allocator past the once-guard; arena bump-allocated so main pointers stay valid; forward_checkpoint re-anchored above both). The worker routes a request to scratch when a much-longer conversation is cached (committed > 2×prompt_len) AND it fits (plen+max_new < 16384) — matches tiny divergent aux requests, NOT fresh turns (prompt≈cached) or continuations (high LCP). `nvfp4_aux_mode` AtomicBool makes lookup return 0 + publish no-op for the aux call, so scratch never touches the conversation cache. Validated: BIG→`hi`(→scratch)→BIG-again now **lcp=512 (HIT)** vs 4 (miss), all outputs coherent (spec works on scratch KV). DEFAULT-ON in code (opt-out `RVLLM_GEMMA4_NVFP4_AUX_KV=0`). Diagnostic `G4N_PREFIX_CACHE_TRACE=1`. Only gemma4-nvfp4 affected. |
