...
extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_gqa_kernel(
    __half*              __restrict__ output,
    const unsigned char* __restrict__ query,
    const uint8_t*       __restrict__ key_cache_packed,
    const uint8_t*       __restrict__ value_cache_packed,
...
extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_gqa_bc16_kernel(
    __half*              __restrict__ output,
    const unsigned char* __restrict__ query,
    const uint8_t*       __restrict__ key_cache_packed,
    const uint8_t*       __restrict__ value_cache_packed,
...
// =====================================================================
// MAX_GQA_DECODE=16 BC=16 variant — head_dim>256 sibling of
// `_gqa_max16_kernel`. Covers Gemma 4 31B global (head_dim=512, GQA=8)
// + any future hd>256 + gqa>4 model on the GQA fast path.
// =====================================================================
#undef MAX_GQA_DECODE
#define MAX_GQA_DECODE 16

extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_gqa_bc16_max16_kernel(
    __half*              __restrict__ output,
    const unsigned char* __restrict__ query,
    const uint8_t*       __restrict__ key_cache_packed,
    const uint8_t*       __restrict__ value_cache_packed,
    ...
    for (int qi = 0; qi < q_len; ++qi) {
        ...
        for (int tile = 0; tile < num_kv_tiles; ++tile) {
            ...
            for (int t = 0; t < tile_len; ++t) {
                ...
                if (tid == 0) {
                    int kv_pos = tile_start + t;
                    // Causal mask + sliding-window mask. Previously
                    // window_size_left was commented out and only
                    // causal applied → sliding layers in the
                    // fallback path attended the full prefix instead
