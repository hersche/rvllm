// K-round batched indirect dual_silu FP8 GEMV for Qwen 3.6 MoE.
//
// Replaces the host-side loop of N=top_k separate launches of
// `fp8_gemv_blockwise_wpr_native_f16in_dual_silu_indirect_kernel`
// with a SINGLE launch whose `grid.z = top_k`. Each block handles
// one (m, n, k_round) triple; the per-row expert id is loaded
// device-side from `top_idx[m * top_k + blockIdx.z]`.
//
// Used in the per-token MoE decode path AND the batched MoE
// prefill path. For per-token decode: grid=(N/8, 1, top_k). For
// batched prefill: grid=(N/8, num_tokens, top_k). Same kernel
// handles both via the `M` arg.
//
// Output layout: `out_silu[(k_round, m, n)] = silu(g) * u` where
// `g = fp8_gemv(w_g[expert_idx], x)`, `u = fp8_gemv(w_u[expert_idx], x)`
// and `silu(z) = z / (1 + exp(-z))`. The k_round stride at the head
// of the `[K, M, N]` shape (k_round-major) lets downstream down-
// projection kernels read silu slices via simple offset addition.
//
// Numerical contract: inner reduction byte-identical to the
// unfused dual_silu_indirect kernel (same fp8 decode, blockscale
// loads, lane stride, warp-shuffle). The only structural change is
// the expert-id lookup (`top_idx[m * top_k + k_round]` instead of
// `*expert_idx_ptr`) and the output stride (`blockIdx.z * M * N +
// m * N + n` instead of `m * N + n`).
//
// Launch geometry: identical block shape to dual_silu_indirect:
//   Grid:  (ceil(N / 8), M, top_k)
//   Block: (256, 1, 1)
//
// Phase 8 dual_silu kround-batch fusion (2026-05-23).

#include <cuda_fp16.h>
#include <math.h>

__device__ __forceinline__ float fp8e4m3_to_float_dskb(unsigned char val) {
...
__device__ __forceinline__ void fp8x2_to_f32_dskb(unsigned short packed_fp8x2,
                                                    float& f0, float& f1) {
    unsigned int f16x2;
...
extern "C"
__global__ void fp8_gemv_blockwise_wpr_native_f16in_dual_silu_indirect_kround_batched_kernel(
    __half* __restrict__       out_silu,        // [top_k, M, N] f16 — k_round-major
    ...
    if (n >= N || m >= M || k_round >= top_k) return;
    ...
    for (int k = lane * 8; k + 7 < K; k += 256) {
        ...
        fp8x2_to_f32_dskb((unsigned short)(wg8),       gw0, gw1);
        fp8x2_to_f32_dskb((unsigned short)(wg8 >> 16), gw2, gw3);
        fp8x2_to_f32_dskb((unsigned short)(wg8 >> 32), gw4, gw5);
        fp8x2_to_f32_dskb((unsigned short)(wg8 >> 48), gw6, gw7);

        float uw0, uw1, uw2, uw3, uw4, uw5, uw6, uw7;
        fp8x2_to_f32_dskb((unsigned short)(wu8),       uw0, uw1);
        fp8x2_to_f32_dskb((unsigned short)(wu8 >> 16), uw2, uw3);
        fp8x2_to_f32_dskb((unsigned short)(wu8 >> 32), uw4, uw5);
        fp8x2_to_f32_dskb((unsigned short)(wu8 >> 48), uw6, uw7);
        ...
        acc_g0 += gw0 * sg0 * x0; acc_g0 += gw1 * sg0 * x1;
        acc_g0 += gw2 * sg0 * x2; acc_g0 += gw3 * sg0 * x3;
        acc_g1 += gw4 * sg4 * x4; acc_g1 += gw5 * sg4 * x5;
        acc_g1 += gw6 * sg4 * x6; acc_g1 += gw7 * sg4 * x7;

        acc_u0 += uw0 * su0 * x0; acc_u0 += uw1 * su0 * x1;
        acc_u0 += uw2 * su0 * x2; acc_u0 += uw3 * su0 * x3;
        acc_u1 += uw4 * su4 * x4; acc_u1 += uw5 * su4 * x5;
        acc_u1 += uw6 * su4 * x6; acc_u1 += uw7 * su4 * x7;
    }
...

    {
        ...
        for (int kr = aligned_k + lane; kr < K; kr += 32) {
            ...
            acc_g += fp8e4m3_to_float_dskb(__ldg(wg_row + kr)) * sg * xv;
            acc_u += fp8e4m3_to_float_dskb(__ldg(wu_row + kr)) * su * xv;
        }
    }
    ...
    if (lane == 0) {
        ...
        long long off = ((long long)k_round * M + m) * N + n;
        out_silu[off] = __float2half(silu * acc_u);
    }
}
