// === Cycle 55 step 10 (Phase F): BF16-OUTPUT VARIANT ===
// Auto-generated bf16-output sibling of
// flash_attention_unified_prefill_nvfp4kv.cu. Internal smem (s_q_f16,
// s_p_f16) stays f16; only the external `output` parameter and the
// epilogue write site flip f16 → bf16.
//
...
// === Task #143 Phase 2 sibling kernel: cp.async-staged K/V load.
//     Diff vs the parent is the K-tile and V-tile load sections only.
//     Kernel symbol differs by `_cpasync_kernel` suffix so the host
//     can dispatch either variant.
extern "C"
__global__ void flash_attention_2_prefill_nvfp4kv_unified_bf16out_cpasync_kernel(
    __nv_bfloat16* __restrict__ output,                 // [num_tokens, H, D]
    ...
    // Task #143 Phase 2: cp.async-staged packed-bytes area for K and
    // V tiles. Each tile = tile_size × (head_dim/2) bytes (NVFP4 is
    // 4 bits/elem). On gemma4-nvfp4 with head_dim=256, tile_size=128
    // → 128 * 128 = 16384 bytes per tile. Allocated AFTER s_acc to
    // not perturb the parent kernel's existing layout addresses.
    // 8-byte aligned (cp.async requires 4/8/16-byte alignment).
    unsigned char* s_k_packed = reinterpret_cast<unsigned char*>(
        s_acc + BLOCK_M * head_dim);
    // Round up s_k_packed start to 8 bytes (s_acc is already at f32
    // = 4-byte alignment; pad if needed).
    {
        uintptr_t a = reinterpret_cast<uintptr_t>(s_k_packed);
        if (a & 7u) s_k_packed += (8u - (a & 7u));
    }
    const int packed_bytes_per_tile = tile_size * (head_dim >> 1);
    unsigned char* s_v_packed = s_k_packed + packed_bytes_per_tile;
    ...
    for (int j = tile_start; j < tile_end; j++) {
        ...
        // -- Load K tile (NVFP4 → f16 smem).  Phase 2 cp.async path:
        //    Pass 1 issues `cp.async.ca.shared.global` for every
        //    (t, block_idx)'s 8-byte packed tile into s_k_packed.
        //    Pass 2 (after wait_group(0)) reads from smem-staged
        //    packed bytes + per-tile scales and dequants into
        //    s_k_f16. Net win = the dequant kernel doesn't stall on
        //    per-thread HBM load latency.
        {
            const int total_units = tile_len * blocks_per_row;
            // Pass 1: async HBM → s_k_packed.
            for (int u = tid; u < total_units; u += FA2_THREADS) {
                ...
                const uint8_t* k_packed_src = key_cache_packed
                    + (slot * num_kv_heads + kv_head) * half_D;
                const int d_base = block_idx << 4;
                const uint8_t* src = k_packed_src + (d_base >> 1);
                unsigned char* dst = s_k_packed + (t * blocks_per_row + block_idx) * 8;
#if __CUDA_ARCH__ >= 800
                unsigned int dst_smem = __cvta_generic_to_shared(dst);
                asm volatile(
...
#endif
            __syncthreads();
            // Pass 2: dequant from smem-staged packed bytes.
            for (int u = tid; u < total_units; u += FA2_THREADS) {
                const int t         = u / blocks_per_row;
                const int block_idx = u - t * blocks_per_row;
                const int kv_pos    = tile_base + t;
                ...
                const int d_base = block_idx << 4;
                uint64_t packed8 = *reinterpret_cast<unsigned long long*>(
                    s_k_packed + (t * blocks_per_row + block_idx) * 8);
        ...
        // -- Load V tile (NVFP4 → f16 smem). cp.async-staged path —
        //    same pattern as the K load above.
        {
            const int total_units = tile_len * blocks_per_row;
            for (int u = tid; u < total_units; u += FA2_THREADS) {
                const int t         = u / blocks_per_row;
                ...
                const int phys_block = block_tables[seq_idx * max_blocks_per_seq + page_idx];
                const int slot      = phys_block * block_size + page_off;
                const uint8_t* v_packed_src = value_cache_packed
                    + (slot * num_kv_heads + kv_head) * half_D;
                const int d_base = block_idx << 4;
                const uint8_t* src = v_packed_src + (d_base >> 1);
                unsigned char* dst = s_v_packed + (t * blocks_per_row + block_idx) * 8;
#if __CUDA_ARCH__ >= 800
                unsigned int dst_smem = __cvta_generic_to_shared(dst);
                asm volatile(
                    "cp.async.ca.shared.global [%0], [%1], 8;"
            ...
            for (int u = tid; u < total_units; u += FA2_THREADS) {
                ...
                const int d_base = block_idx << 4;
                uint64_t packed8 = *reinterpret_cast<unsigned long long*>(
                    s_v_packed + (t * blocks_per_row + block_idx) * 8);
    ...
    for (int idx = tid; idx < BLOCK_M * head_dim; idx += FA2_THREADS) {
        ...
        output[(tok * num_heads + q_head) * head_dim + d] =
            __float2bfloat16(s_acc[idx] * inv_l);
    }
}
