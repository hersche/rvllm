...
extern "C"
__global__ void flash_attention_2_prefill_nvfp4kv_unified_nosvt_kernel(
    ...
    if (window_size_left >= 0) {
        ...
        // Codex18-2: gate is `>= 0` (was `> 0`) — decode treats `< 0`
        // as the lone "no window" sentinel; `window_size_left == 0`
        // means "only current token" and must mask everything else.
    ...
    // _nosvt variant: s_v_f16_T eliminated; P·V reads V row-major
    // directly. Saves MMA_K * head_dim * 2 bytes (8 KB at hd=256).
    ...
    for (int j = tile_start; j < tile_end; j++) {
        ...
        const auto mask_pack = [&](int m, int t, float dot) -> float {
            ...
            // Codex18-2: `< 0` is the "no window" sentinel (matches
            // decode); `0` means only-current-token, so the (delta<=W)
            // branch handles it (delta>0 → masked).
        ...
        for (int ks_i = 0; ks_i < k_sub_count; ks_i++) {
            ...
            // (1) _nosvt: NO transpose pass. V is read row-major from
            //     s_v_f16 directly in the P·V MMA via software re-pack
            //     below. Saves MMA_K * head_dim/FA2_THREADS dequant+store
            //     iterations per ks_i + MMA_K*head_dim*2 bytes of smem.
            ...
            for (int nt = 0; nt < n_tiles_per_warp; nt++) {
                ...
                // _nosvt re-pack: B frag for P·V MMA n8k16 from V
                // row-major [tile_size, head_dim]. Per-lane reads 4
                // separate u16 (lane.n = lane>>2, lane.k ∈
                // {(lane&3)*2+0, +1, +8, +9}; d = n_base + lane.n)
                // and combines into 2 packed u32 pairs matching
                // pack_b_frag_col_major_n8k16_f16's register layout.
                uint32_t b[2];
                {
                    const int lane_n = lane >> 2;
                    const int k_lo   = (lane & 3) << 1;
                    const int k_hi   = k_lo + 8;
                    const int d      = n_base + lane_n;
                    const int row_stride = head_dim;
                    const __half* base = s_v_f16 + d;
                    auto ld = [&](int k_off) -> unsigned short {
                        const int t_src = k_base_t + k_off;
                        if (t_src >= tile_len) return (unsigned short)0;
                        __half v = base[t_src * row_stride];
                        return *reinterpret_cast<unsigned short*>(&v);
                    };
                    unsigned short v0 = ld(k_lo);
                    unsigned short v1 = ld(k_lo + 1);
                    unsigned short v2 = ld(k_hi);
                    unsigned short v3 = ld(k_hi + 1);
                    b[0] = (uint32_t(v1) << 16) | uint32_t(v0);
                    b[1] = (uint32_t(v3) << 16) | uint32_t(v2);
                }
