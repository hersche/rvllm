...
__global__ void flash_attention_2_decode_nvfp4kv_gqa_bf16out_kernel(
    ...
    if (context_len == 0) {
        // Codex36-2: GQA bf16-out BC=32 padded-zero.
        for (int q = 0; q < GQA; ++q) {
            int h = kv_head * GQA + q;
...
// =====================================================================
// MAX_GQA_DECODE=16 bf16-out BC=32 variant — sibling of the bf16-out
// _gqa_bf16out_kernel above + the f16-out _gqa_max16_kernel in
// flash_attention_nvfp4kv.cu. Same kernel body, larger register-array
// cap. Covers Mistral 3.5 / Gemma 4 31B / Qwen 3.6 35B-A3B on the
// bf16-residual code path.
// =====================================================================
#undef MAX_GQA_DECODE
#define MAX_GQA_DECODE 16

extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_gqa_max16_bf16out_kernel(
    __nv_bfloat16* __restrict__ output,
    const unsigned char* __restrict__ query,
    const uint8_t*       __restrict__ key_cache_packed,
    const uint8_t*       __restrict__ value_cache_packed,
...
__global__ void flash_attention_2_decode_nvfp4kv_gqa_bc16_bf16out_kernel(
    ...
    const int GQA = (num_kv_heads > 0) ? (num_heads / num_kv_heads) : 0;
    if (context_len == 0) {
        // Codex36-2: GQA bf16-out BC=16 padded-zero.
        for (int q = 0; q < GQA; ++q) {
            int h = kv_head * GQA + q;
            if (h >= num_heads) break;
            const int out_base = (seq_idx * num_heads + h) * head_dim;
...
// =====================================================================
// MAX_GQA=16 BC=16 bf16-out variant — head_dim>256 + bf16 residual
// + GQA>4. Sibling of the f16-out _gqa_bc16_max16_kernel.
// =====================================================================
#undef MAX_GQA_DECODE
#define MAX_GQA_DECODE 16

extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_gqa_bc16_max16_bf16out_kernel(
    __nv_bfloat16*       __restrict__ output,
    const unsigned char* __restrict__ query,
    const uint8_t*       __restrict__ key_cache_packed,
    const uint8_t*       __restrict__ value_cache_packed,
