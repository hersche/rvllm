...
// =====================================================================
// MAX_GQA_SPLIT=16 variant — covers Mistral 3.5 GQA=12 + Gemma 4 31B
// global GQA=8 + Qwen 3.6 35B-A3B GQA=8 on the split-decode path.
// Body identical to the kernel above; only the register-array cap
// changes via #define toggle. Host dispatcher (`decode.rs`) picks
// this variant when actual GQA exceeds the default cap of 8.
// =====================================================================
#define MAX_GQA_SPLIT 16
extern "C"
__global__ void flash_attention_2_decode_nvfp4kv_split_gqa_max16_kernel(
    float*               __restrict__ tmp_out,
    float*               __restrict__ max_logits,
    float*               __restrict__ exp_sums,
    const unsigned char* __restrict__ query,
